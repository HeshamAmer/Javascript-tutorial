...
Book.loadAll = function () {
  ...
  if (bookTableString) {
    ...
    for (i=0; i < keys.length; i++) {
      key = keys[i];
      Book.instances[key] =  bookTable[key];
    }
  }
};
...
Book.setupUserInterface=function(){
...
function setupUserInterface(){
  ...
  var column=document.createElement("td");
    ...
    for (var i=0;i<numberOfBooks;i++){
        ...
        column.innerText=Book.instances[Object.keys(Book.instances)[i]].isbn;
        row.appendChild(column);
        column=document.createElement("td");
        column.innerText=Book.instances[Object.keys(Book.instances)[i]].title;
        row.appendChild(column);
        column=document.createElement("td");
        column.innerText=Book.instances[Object.keys(Book.instances)[i]].year;
        row.appendChild(column);
        table.appendChild(row);
        console.log(i);
    }
